...
void cargar_todo(String vt1,String vc,String vp,String vu,String va,String an){
  ...
  total=-1;         
    ...
    for(int i=0;i<vt2.length-1;i++){   
      vt3 =splitTokens(vt2[i],";");        
      total+=1;
      if(vp==vt3[9]){ for(int j=0;j<anio.length;j++){ if(vt3[9]==anio[j]){ totan[j]+=1; } } }  
...
void cargar_resultados(String vr1,String vc,String vp,String vu,String va,String an){     
    ...
    if(vc=="total"&&vp=="todo"){ 
       for(int i=0;i<totan.length;i++){ totan[i]=0; }  total=-1;
     }else{
       for(int i=0;i<tan.length;i++){ tan[i]=0; }  totf=0;   
     }
    ...
    for(int i=0;i<vr2.length-1;i++){   
      ...
      if(vc=="total"&&vp=="todo"){  
         total+=1; 
         for(int j=0;j<anio.length;j++){ if(vr3n[9]==anio[j]){ totan[j]+=1; } } 
       }else{  
         totf+=1; 
         for(int j=0;j<anio.length;j++){ if(vr3n[9]==anio[j]){ tan[j]+=1; } }   
      }
...
void car_vis(String vc,String vp,String an,String titu){     
  ...
  esp=height/total_paises.length;
  ...
  for(int j=0;j<anio.length;j++){ 
    ...
    if(anio[j]==an){ 
      fill(#333399); rect(30,(12*j),-an_sel[j],10); 
...
//---------  
  for(int i=0;i<total_paises.length-1;i++){          
     if(my>esp*i&&my<esp+(esp*i)&&mx>0&&mx<220){
...
//--------bot-años-fecha-------------
  ...
  fill(350,30); 
  for(int j=0;j<anio.length;j++){ 
   if(mx>botx[2]&&mx<botx[2]+100&&my>boty[2]+(12*j)-5&&my<boty[2]+(12*j)+5){
